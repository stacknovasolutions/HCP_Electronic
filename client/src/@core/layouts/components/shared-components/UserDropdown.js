  ...
  return (
    <Fragment>
      ...
      >
        <Box sx={{ pt: 2, pb: 3, px: 4 }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            <Box sx={{ display: 'flex', ml: 3, alignItems: 'flex-start', flexDirection: 'column' }}>
              <Typography sx={{ fontWeight: 600 }}>{JSON.parse(localStorage.getItem("userData"))?.FirstName + " " + JSON.parse(localStorage.getItem("userData"))?.LastName}</Typography>
        ...
        <MenuItem sx={{ p: 0 }}>
          <Box onClick={() => handleDropdownClose('/apps/profile')} sx={styles}>
